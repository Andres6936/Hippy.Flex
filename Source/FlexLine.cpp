  ...
  for (size_t i = 0; i < items.size(); i++) {
    ...
    float flexFactor =
        flexSign == PositiveFlexibility ?
            item->style.flexGrow : item->style.flexShrink;
    if (flexFactor == 0
        || (flexSign == PositiveFlexibility
...
void FlexLine::FreezeViolations(std::vector<HPNode*>& violations) {
  ...
  for (size_t i = 0; i < violations.size(); i++) {
    ...
    if (item->isFrozen)
      continue;
    remainingFreeSpace -= (item->getLayoutDim(mainAxis)
        - item->result.hypotheticalMainAxisSize);
    totalFlexGrow -= item->style.flexGrow;
    totalFlexShrink -= item->style.flexShrink;
    totalWeightedFlexShrink -= item->style.flexShrink
...
bool FlexLine::ResolveFlexibleLengths() {
  ...
  for (size_t i = 0; i < items.size(); i++) {
    ...
    if (remainingFreeSpace > 0 && totalFlexGrow > 0
      ...
      extraSpace = remainingFreeSpace * item->style.flexShrink
          * item->result.flexBaseSize / totalWeightedFlexShrink;
    }
    ...
    if (std::isfinite(extraSpace)) {
      ...
      usedFreeSpace += adjustItemMainSize
          - item->result.hypotheticalMainAxisSize;
...
//	}
  ...
  for (int i = 0; i < itemsSize; i++) {
    ...
    item->setLayoutEndPosition(
        mainAxis,
        flexContainer->getLayoutDim(mainAxis) - item->getLayoutDim(mainAxis)
            - offset);
    offset += item->getLayoutDim(mainAxis) + item->getLayoutEndMargin(mainAxis)
        + space;
  }
}
